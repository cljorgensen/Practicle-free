...
<script>
	$(document).ready(function() {
		<?php initiateAdancedViewTable("TableITSM"); ?>
	});
				...
				<div class="card-body">
					...
					<table id="TableITSM" class="table table-responsive table-borderless table-hover" cellspacing="0">
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							$row['CreatedByFullName'] = getFullNameFromID($row['CreatedBy']);
							$row['LastEditedByFullName'] = getFullNameFromID($row['LastEditedBy']);
						...
						<tbody>
							<?php foreach ($tempArray as $row) { ?>
								<tr>
									...
									?>
									<?php $CreatedBy = getFullNameFromID($row['CreatedBy']); ?>
									<?php $LastEditedBy = getFullNameFromID($row['LastEditedBy']); ?>
									<?php
