                ...
                <div class="card-body">
                    ...
                    <script>
                        $(document).ready(function() {
                            <?php initiateAdvancedViewTable("TableChangelog"); ?>
                        });
                    </script>
                        ...
                        <tbody>
                            <?php while ($row = mysqli_fetch_array($result)) { ?>
                                ...
                                if (in_array("100000", $_SESSION['memberofgroups'])) {
                                    $ChangeLogID = $row['ID'];
                                    $editLink = "<a href=\"#\" data-bs-toggle=\"modal\" data-bs-target=\"#modalEditChangelog\" onclick=\"runModalEditChangelog('$ChangeLogID')\"><i class=\"fa-solid fa-pen-to-square\"></i></a>";
                                } else {
                                    $editLink = "";
                                }
                                ?>
                                <tr class='text-sm text-secondary mb-0 text-wrap'>
                                    <td><?php echo $editLink . " " . $row['Version']; ?></td>
                                    <td><?php echo convertToDanishDateFormat($row['ChangelogDate']) ?></td>
