...
function getRelatedSLAID($CompanyID)
{
    global $conn;
    global $functions;
    $sql = "SELECT RelatedSLAID FROM Companies WHERE ID = $CompanyID;";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    while ($row = $result->fetch_assoc()) {
        $RelatedSLAID = $row['RelatedSLAID'];
    }
    return $RelatedSLAID;
}
...
function updateSettingValue($ID,$Value)
{
    global $conn;
    global $functions;
    $sql = "UPDATE settings SET SettingValue = $Value WHERE ID = $ID;";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
...
function createTicketLogEntry($TicketID, $UserID, $LogTypeID, $LogActionText)
{
    global $conn;
    global $functions;
    $sql = "INSERT INTO logtickets (LogTypeID, LogActionText, LogActionDate, RelatedTicketID, UserID) VALUES ('$LogTypeID','$LogActionText',NOW(),'$TicketID','$UserID');";
...
function createTimelineSLAViolationDates($ElementID, $ModuleID, $RelatedStatusCodeID, $SLAViolationDate)
{
    ...
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("iiis", $ElementID, $ModuleID, $RelatedStatusCodeID, $SLAViolationDate);
        if (!$stmt->execute()) {
            die('Query fail: ' . $stmt->error);
        }
        $stmt->close();
    } else {
...
function createNewProject($ProjectManager, $ProjectStatus, $ProjectStart, $ProjectDeadline, $ProjectName, $ProjectDescription, $ProjectRelCustomer, $ProjectResponsible)
{
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        mysqli_stmt_bind_param($stmt, "ssssssssi", $ProjectName, $ProjectStart, $ProjectStatus, $ProjectDeadline, $ProjectDescription, $ProjectManager, $ProjectRelCustomer, $ProjectResponsible, $UserID);
        ...
        createProjectBaselineLog($last_id);
    }
...
function createNewProjectTask($ProjectTaskEstimatedHours, $TaskResponsible, $ProjectTaskStatus, $ProjectTaskParent, $ProjectTaskStart, $ProjectTaskDeadline, $ProjectTaskEstimatedBudget, $ProjectTaskBudgetSpend, $ProjectTaskName, $ProjectTaskDescription, $ProjectID, $ProjectRelatedCategory, $ProjectTaskProgress, $ProjectPrivate)
{
    ...
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("sssssssiiiiiii", $ProjectID, $ProjectTaskName, $ProjectTaskDescription, $ProjectTaskStart, $ProjectTaskDeadline, $TaskResponsible, $ProjectTaskStatus, $ProjectTaskParent, $ProjectTaskEstimatedBudget, $ProjectTaskEstimatedHours, $ProjectTaskBudgetSpend, $ProjectTaskProgress, $ProjectRelatedCategory, $ProjectPrivate);
        $stmt->execute();
        if ($stmt->error) {
            $functions->errorlog($stmt->erro,"createNewProjectTask");
        }
        $last_id = $conn->insert_id;
        $stmt->close();
...
function removeRelatedSprintID($SprintID)
{
    ...
    $sql = "UPDATE project_tasks SET RelatedSprintID = NULL WHERE RelatedSprintID = '$SprintID'";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function updateProjectTask($ProjectID, $ProjectTaskID, $TaskResponsible, $ProjectTaskStatus, $ProjectTaskParent, $ProjectTaskStart, $ProjectTaskDeadline, $ProjectTaskEstimatedBudget, $ProjectTaskBudgetSpend, $ProjectTaskEstimatedHours, $ProjectTaskHoursSpend, $ProjectRelatedCategory, $ProjectTaskProgress, $ProjectTaskName, $ProjectTaskDescription, $ProjectPrivate)
{
    ...
    if ($ProjectTaskEstimatedHours == "") {
        $ProjectTaskEstimatedHours = 0;
    }

    if ($ProjectTaskHoursSpend == "") {
        $ProjectTaskHoursSpend = 0;
    }

    // Prepare the SQL statement
    $stmt = $conn->prepare("UPDATE project_tasks
...
function updateProject($ProjectName, $ProjectStatus, $ProjectRelCustomer, $ProjectDescription, $ProjectStart, $ProjectDeadline, $ProjectManager, $ProjectResponsible, $ProjectID)
{
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        // Bind parameters to the prepared statement
        mysqli_stmt_bind_param($stmt, "ssisssssii", $ProjectName, $ProjectStatus, $ProjectRelCustomer, $ProjectStart, $ProjectDeadline, $ProjectDescription, $ProjectManager, $ProjectResponsible, $UserID, $ProjectID);
...
function deleteWorkFlowStep($StepID)
{
    ...
    $sql = "DELETE FROM workflowsteps_template
                WHERE workflowsteps_template.ID='" . $StepID . "';";

    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function createNewPasswordEntry($RelatedCompanyID, $ServerDestination, $ServerDomain, $Username, $password, $TypeID, $Description, $RelatedGroupID)
{
    global $conn;
    global $functions;
    $dbfull = isDBFull();
    if ($dbfull == "Yes") {
        ...
        $sql = "INSERT INTO passwordmanager_passwords (RelatedCompanyID, ServerDestination, Domain, Username, Password, DestinationTypeID, Description, RelatedGroupID)
                    VALUES ($RelatedCompanyID,'$ServerDestination','$ServerDomain','$Username','$password',$TypeID,'$Description',$RelatedGroupID);";
        mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    }
}

function updatePasswordEntry($PasswordID, $RelatedCompanyID, $ServerDestination, $ServerDomain, $Username, $password, $TypeID, $Description, $RelatedGroupID)
{
    global $conn;
    global $functions;
    $Description = mysqli_real_escape_string($conn, $Description);

    $sql = "UPDATE passwordmanager_passwords SET RelatedCompanyID='$RelatedCompanyID',ServerDestination='$ServerDestination',Domain='$ServerDomain',Username='$Username',Password='$password',DestinationTypeID='$TypeID',Description='$Description',RelatedGroupID='$RelatedGroupID'
                WHERE passwordmanager_passwords.ID = $PasswordID;";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function deletePasswordEntry($PasswordID)
{

    global $conn;
    global $functions;

    $sql = "DELETE FROM passwordmanager_passwords WHERE passwordmanager_passwords.ID='" . $PasswordID . "';";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function incrementDocumentVersion($ITSMTypeID, $ITSMID)
{
    ...
    $updateSql = "UPDATE $ITSMTableName SET Version = Version + 1 WHERE ID = $ITSMID;";
    mysqli_query($conn, $updateSql) or die('Query fail: ' . mysqli_error($conn));
...
function getChatRoomName($RoomID)
{
    ...
    $sql = "SELECT name FROM chatrooms WHERE ID = $RoomID";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
...
function updateString($DocumentID, $DocContent)
{
    global $conn;
    global $functions;

    $sql = "UPDATE knowledge_documents SET content = '$DocContent' WHERE ID = $DocumentID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function makereservation($ITSMTypeID, $UserID)
{
    global $conn;
    global $functions;
    ...
    switch ($ITSMTypeID) {
        ...
        case "2":
            ...
            break;
        case "3":
            $TableSelect = "knowledge_documents";
            break;
        case "5":
            $TableSelect = "problems";
            break;
        case "6":
            $TableSelect = "projects";
            break;
        case "7":
            $TableSelect = "requests";
            break;
        case "13":
            $TableSelect = "project_tasks";
            break;
        default:
            echo "None";
    }

    //Clean up old records from reservations
    $sql = "DELETE FROM module_reservations WHERE date < DATE(NOW() - INTERVAL 1 DAY);";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    $sql = "DELETE FROM module_reservations WHERE ReservedBy = $UserID AND ModuleType = $ITSMTypeID;";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

    //Get Current Module Max ID
    $sql = "SELECT MAX(ID) AS ID from $TableSelect;";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    while ($row = mysqli_fetch_array($result)) {
        $MaxID = $row['ID'];
    }

    //Get a count on number of reservations
    $sql = "SELECT COUNT(ID) AS Antal FROM module_reservations WHERE ModuleType = $ITSMTypeID;";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    while ($row = mysqli_fetch_array($result)) {
        $Reservations = $row['Antal'];
    }

    $ReservedID = ($MaxID + 1) + $Reservations;

    $sql = "INSERT INTO module_reservations (ModuleType,ReservedID,ReservedBy) VALUES ($ITSMTypeID,$ReservedID,$UserID);";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

    return $ReservedID;
}

function uploadModuleClipboardToRelatedModule($ModuleID, $ReservedID, $UserID)
{
    global $conn;
    global $functions;
    ...
    switch ($ModuleID) {
        case "1":
            $UploadFolder = "./uploads/files_tickets";
            $TableSelect = "files_tickets";
            break;
        case "2":
            $UploadFolder = "./uploads/files_changes";
            $TableSelect = "files_changes";
            break;
        case "3":
            $UploadFolder = "./uploads/files_documents";
            $TableSelect = "files_documents";
            break;
        case "5":
            $UploadFolder = "./uploads/files_problems";
            $TableSelect = "files_problems";
            break;
        case "6":
            $UploadFolder = "./uploads/files_projects";
            $TableSelect = "files_projects";
            break;
        case "7":
            $UploadFolder = "./uploads/files_requests";
            $TableSelect = "files_requests";
            break;
        case "13":
            $UploadFolder = "./uploads/files_projecttasks";
            $TableSelect = "files_projecttasks";
            break;
        default:
            echo "None";
    }
    ...
    // Create Filename
    $FileName = $ModuleID . "-" . $ReservedID . "--" . $UserID . ".png";
    $SourceFile = "./uploads/clipboardstemp/$FileName";
    $DestinationFile = "$UploadFolder/$FileName";
    if (file_exists($SourceFile)) {
        // SQL insert row on module related upload files 
        $sql = "INSERT INTO $TableSelect(FileName, FileNameOriginal, RelatedElementID, Date, RelatedUserID) VALUES ('$FileName','$FileName',$ReservedID,Now(),$UserID)";
        mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

        // Copy file from clipboad temp upload folder to module destination folder
        rename("$SourceFile", "$DestinationFile");
    } else {
    }

    /*
        $files = glob("../uploads/images/clipboardstemp/*.png");

        foreach ($files as $file) {
            $ModuleType = strtok($mystring, '-');
            echo $file;
            $str = $file;
            ...
            $FileName = $ModuleType."-".$ReservedID."--".$UserID.".png";

            $sql = "INSERT INTO $UploadFolder(FileName, FileNameOriginal, RelatedTicketID, Date, RelatedUserID) VALUES ('$FileName','$FileName',$ReservedID,'Now()',$UserID)";

            rename("../uploads/images/clipboardstemp/$file","$UploadFolder/$file");
        }
        */
}

function deleteModuleReservations($ModuleID, $ReservedID, $UserID)
{
    global $conn;
    global $functions;

    $sql = "DELETE FROM module_reservations WHERE ReservedBy = $UserID AND ModuleType = $ModuleID;";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function initiateAdvancedtrumbowyg($fieldname)
{

    echo "$('#$fieldname').trumbowyg({
            btns: 
            [
                ['viewHTML'],
                ['fontfamily'],
...
function initiateSimpleTrumbowyg($fieldname)
{
...
function pullIncident($IncidentID, $UserID, $TeamID)
{
    global $conn;
    global $functions;

    $sql = "UPDATE tickets SET ResponsibleTeam=$TeamID, Responsible=$UserID WHERE tickets.ID = $IncidentID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function pullproblem($ProblemID, $UserID, $TeamID)
{
    global $conn;
    global $functions;

    $sql = "UPDATE problems SET ResponsibleTeam=$TeamID, Responsible=$UserID WHERE problems.ID = $ProblemID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function pullRequest($RequestID, $UserID, $TeamID)
{
    global $conn;
    global $functions;

    $sql = "UPDATE requests SET ResponsibleTeam=$TeamID, Responsible=$UserID WHERE tickets.ID = $RequestID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function createFormsTable($FormID, $FormsTableName)
{
    ...
    $sql = "INSERT INTO forms_tables(RelatedFormID, TableName) VALUES ($FormID,'$FormsTableName')";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function deleteFormsTable($TableID)
{
    ...
    $sql = "DELETE FROM forms_tables WHERE ID = $TableID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function createFormsTableField($FieldLabel, $FieldName, $FieldType, $TableID, $FieldLength, $FieldWidth)
{
    ...
    $sql = "INSERT INTO forms_tables_fieldslist(RelatedTableID, FieldName, Label, FieldType, FieldLength, FieldWidth) VALUES ($TableID,'$FieldName','$FieldLabel',$FieldType,$FieldLength,$FieldWidth)";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function createDatabaseTableField($fieldlabel, $fieldname, $fieldtype, $tableid, $fieldlength)
{
    ...
    $sql = "ALTER TABLE $TableName
                ADD COLUMN $fieldname $DBFieldDef($fieldlength);";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function deleteFormsTableField($FieldID)
{
    ...
    $sql = "DELETE FROM forms_tables_fieldslist WHERE ID = $FieldID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function checkifServerExists($Name, $ModelNo)
{
    global $conn;
    global $functions;

    $ConcatID1 = $Name . $ModelNo;
    $ConcatID2 = "";
    $sql = "SELECT CONCAT(ci_servers.Hostname,'',ci_servers.ModelNo) AS ConcatID
                FROM ci_servers
                WHERE ci_servers.Hostname = '$Name'";

    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    while ($row = mysqli_fetch_array($result)) {
        $ConcatID2 = $row['ConcatID'];
    }
    if (strcmp($ConcatID1, $ConcatID2) !== 0) {
        $Result = "0";
    } else {
        $Result = "1";
    }
...
function checkifWorkstationsExists($Name, $ModelNo)
{
    global $conn;
    global $functions;
    ...
    $ConcatID1 = $Name . $ModelNo;
    $ConcatID2 = "";
    $sql = "SELECT CONCAT(ci_workstations.Hostname,'',ci_workstations.ModelNo) AS ConcatID
                FROM ci_workstations
                WHERE ci_workstations.Hostname = '$Name'";
...
function checkifHandheldsExists($ModelNo, $SerialNumber)
{
    global $conn;
    global $functions;

    $ConcatID1 = $ModelNo . $SerialNumber;
    $ConcatID2 = "";
    $sql = "SELECT CONCAT(ci_handhelds.ModelNo,'',ci_handhelds.SerialNumber) AS ConcatID
                FROM ci_handhelds
                WHERE ci_handhelds.SerialNumber = '$SerialNumber'";
...
function checkifSubscriptionExists($MobilePhoneNumber, $SIM)
{
    global $conn;
    global $functions;

    $ConcatID1 = $MobilePhoneNumber . $SIM;
    $ConcatID2 = "";
    $sql = "SELECT CONCAT(ci_mobilesubscriptions.MobilePhoneNumber,'',ci_mobilesubscriptions.SIM) AS ConcatID
                FROM ci_mobilesubscriptions
                WHERE ci_mobilesubscriptions.MobilePhoneNumber = '$MobilePhoneNumber'";
...
function checkifCertificateExists($Name, $Active)
{
    global $conn;
    global $functions;

    $ConcatID1 = $Name . $Active;
    $ConcatID2 = "";
    $sql = "SELECT CONCAT(cis.Name,'',cis.Active) AS ConcatID
                FROM cis
                WHERE cis.Name = '$Name'";
...
function getNewlyCreateChange($CreatedBy)
{
    global $conn;
    global $functions;

    $sql = "SELECT MAX(ID) AS ChangeID
                FROM changes
...
function getChangeCABPerson($CABID)
{
    global $conn;
    global $functions;

    $sql = "SELECT MAX(UserID) AS CABUserID
                FROM cab_users
                WHERE CABID = $CABID";

    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    while ($row = mysqli_fetch_array($result)) {
        $CABUserID = $row['CABUserID'];
    }
    return $CABUserID;
}

function createStandardChangeTemplate($UserID, $Name, $Description)
{
    global $conn;
    global $functions;
    $sql = "INSERT INTO changes_standards(Name, Description, CreatedBy) VALUES ('$Name','$Description',$UserID)";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function createStandardRequestTemplate($UserID, $Name, $Description)
{
    global $conn;
    global $functions;
    $sql = "INSERT INTO requests_templates(Name, Description, CreatedBy) VALUES ('$Name','$Description',$UserID)";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function updatePinForUser($UserID, $Pin)
{
    global $conn;
    global $functions;
    $sql = "UPDATE users SET Pin = '$Pin' WHERE ID = $UserID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function createFieldForStandardChangeTemplate($FieldName, $FieldContent, $StandardChangeID)
{
    global $conn;
    global $functions;
    $sql = "INSERT INTO changes_standard_fields (RelatedStdChangeID, FieldName, FieldContent) VALUES ('$StandardChangeID','$FieldName','$FieldContent')";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function createChangeRelation($ChangeID, $ElementID, $Elementtype)
{
    global $conn;
    global $functions;
    $RelationsTable = "";
    ...
    if ($Elementtype == 1) {
        $RelationsTable = "ticketchanges";
        $FieldName = "TicketID";
    }

    if ($Elementtype == 5) {
        $RelationsTable = "changes_problems";
        $FieldName = "ProblemID";
    }

    if ($Elementtype == 6) {
        $RelationsTable = "projectschanges";
        $FieldName = "ProjectID";
    }

    $sql = "INSERT INTO $RelationsTable(ChangeID, $FieldName) VALUES ($ChangeID,$ElementID)";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function createRequestRelation($RequestID, $ElementID, $Elementtype)
{
    global $conn;
    global $functions;
    $RelationsTable = "";
    ...
    if ($Elementtype == 1) {
        $RelationsTable = "ticketrequests";
        $FieldName = "TicketID";
    }

    if ($Elementtype == 2) {
        $RelationsTable = "requestchanges";
        $FieldName = "ChangeID";
    }

    if ($Elementtype == 5) {
        $RelationsTable = "requestproblems";
        $FieldName = "ProblemID";
    }

    if ($Elementtype == 6) {
        $RelationsTable = "requestprojects";
        $FieldName = "ProjectID";
    }

    if ($Elementtype == 13) {
        $RelationsTable = "requestprojectstasks";
        $FieldName = "ProjectTaskID";
    }

    $sql = "INSERT INTO $RelationsTable(RequestID, $FieldName) VALUES ($RequestID,$ElementID)";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}

function createProblemRelation($ProblemID, $ElementID, $ElementType)
{
    global $conn;
    global $functions;
    $RelationsTable = "";
    ...
    if ($ElementType == 1) {
        $RelationsTable = "ticketproblems";
        $FieldName = "TicketID";
    }

    if ($ElementType == 2) {
        $RelationsTable = "changesproblems";
        $FieldName = "ChangeID";
    }
    ...
    if ($ElementType == 6) {
        $RelationsTable = "projectsproblems";
        $FieldName = "ProjectID";
    }

    if ($ElementType == 13) {
        $RelationsTable = "projecttasksproblems";
        $FieldName = "ProjectTaskID";
    }

    $sql = "INSERT INTO $RelationsTable(ProblemID, $FieldName) VALUES ($ProblemID,$ElementID)";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function setLockStatus($UserID)
{
    ...
    $sql = "UPDATE sessions SET locked = 1 WHERE userid = $UserID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function getTicketPriorityName($PriorityID)
{
    global $conn;
    global $functions;
...
function getRequestPriorityName($PriorityID)
{
    global $conn;
    global $functions;
...
function updateTaskFromWorkflowStep($TaskID, $UsersID, $ModalDeadline, $Status)
{
    ...
    $sql = "UPDATE taskslist SET RelatedUserID=$UsersID,Deadline='$ModalDeadline',Status='$Status'
                WHERE ID = $TaskID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function createNewTicketFromEmail($Subject, $Priority, $MessageFullText, $CompanyID, $CustomerID, $ITSMTypeID){
    global $conn;
    global $functions;
    ...
    $sql = "INSERT INTO `$ITSMTableName`(`RelatedCompanyID`, `Customer`, `Subject`, `Description`, `Status`, `Priority`, `SLA`, `Created`, `CreatedBy`, `LastUpdated`) 
            VALUES ('$CompanyID','$CustomerID','$Subject','$MessageFullText','1','$Priority','$SLA',NOW(),'$CustomerID',NOW())";

    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

    $last_itsm_id = mysqli_insert_id($conn);

    $LogActionText = "Created from email";
    createITSMLogEntry($last_itsm_id, $ITSMTypeID, $CustomerID, $LogActionText);
    ...
    $ITSMTypeName = $functions->translate($ITSMTypeName);
    $MailCode = getMailCodeForModule($ITSMTypeID);
    $MailTemplateSubject = getMailTemplateSubject(3);

    $InternalViewLink = "<a href=\"$SystemURL/itsm-$ITSMTypeID-$last_itsm_id\">" . $functions->translate("Open") . " $ITSMTypeName: $last_itsm_id</a>";

    $MailTemplateSubject = str_replace("<:mailcode:>", $MailCode, $MailTemplateSubject);
    $MailTemplateSubject = str_replace("<:itsmnumber:>", $last_itsm_id, $MailTemplateSubject);
    $MailTemplateSubject = str_replace("<:systemname:>", $SystemName, $MailTemplateSubject);
    
    $MailTemplateContent = getMailTemplateContent(3);
    ...
    $MailTemplateContent = str_replace("<:itsmnumber:>", $last_itsm_id, $MailTemplateContent);
    $MailTemplateContent = str_replace("<:content:>", $MessageFullText, $MailTemplateContent);
    $MailTemplateContent = str_replace("<:systemname:>", $SystemName, $MailTemplateContent);
    $MailTemplateContent = str_replace("<:systemurl:>", $SystemURL, $MailTemplateContent);
    $MailTemplateContent = str_replace("<:itsmtypename:>", $ITSMTypeName, $MailTemplateContent);
    $MailTemplateContent = str_replace("<:internalviewlink:>", $InternalViewLink, $MailTemplateContent);
...
function createITSMComment($ITSMID, $ITSMTypeID, $Comment, $UserID)
{
    ...
    if($CustomerID == $UserID){
        $sql = "INSERT INTO itsm_comments (RelatedElementID, ITSMType, UserID, Text, Internal) VALUES ('$ITSMID', '$ITSMTypeID', '$UserID', '$Comment', '0');";
        $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    }    
}
...
function getMailsAndCreateTickets()
{
    ...
    require_once(ABSPATH . '../classes/imap/class.imap.php');
    ...
    foreach ($inbox['data'] as $emails) {
        ...
        if (!empty($UID)) {
            $EmailExists = "1";
            $EmailAddress = $emails['from']['address'];
            ...
            $RelatedUserTypeID = "2";
            $RelatedManagerID = "";
            $hashed_password = $hashed_password = $functions->SaltAndHashPasswordForCompare("xUG12Gt64");
            ...
            if ($EmailExists == "1") {
                ...
                $MessageFullText = strip_tags($Message);
                $RelatedCustomerID = checkIfUserExists($EmailAddress);
                $CompanyID = checkEmailForValidSubdomain($EmailAddress);
                if (!empty($RelatedCustomerID)) {
                    //User exists continue
                    createNewTicketFromEmail($Subject, $Type, $Status, $Priority, $Message, $MessageFullText, $CompanyID, $RelatedCustomerID);
                    removeIMAPEmail($UID);
                } else {
                    //User does not exist
                    $CompanyID = checkEmailForValidSubdomain($EmailAddress);
                    if (empty($CompanyID)) {
                        //email not valid for existing customer
                        exit;
                    } else {
                        //email is valid - lets create user for the company
                        //Lets get decent name defined
                        $parts = explode(' ', $Name);
                        if (count($parts) === 2) {
                        ...
                        $RelatedCustomerID = createNewUser($firstname, $lastname, $EmailAddress, $EmailAddress, $hashed_password, $CompanyID, $JobTitel, $RelatedUserTypeID, $RelatedManagerID, $StartDate, $NewPin);
                        createNewTicketFromEmail($Subject, $Type, $Status, $Priority, $Message, $MessageFullText, $CompanyID, $RelatedCustomerID);
                        removeIMAPEmail($UID);
                    }
                }
...
function doesORSiteExist($SiteName)
{
    global $conn;
    global $functions;
...
function registerDatabaseBackup($ITSMTypeID, $Backup_File_Name, $UserID, $Date, $Description)
{
    ...
    // Bind the parameters
    // The types are 'ssiss': s=string, i=integer. Based on the column data types in your table.
    $stmt->bind_param('ssiss', $Date, $Description, $UserID, $ITSMTypeID, $Backup_File_Name);
    ...
    if ($stmt->execute()) {
        ...
        $error = 'Statement failed: ' . $stmt->error;
        $functions->errorlog($error,"registerDatabaseBackup");
...
function updateStatusOnElement($ElementID, $TableName, $Status)
{
    global $conn;
    global $functions;
...
function getModuleIDFromKanbanTask($KanbanTaskID)
{
    global $conn;
    global $functions;
    ...
    return $RelatedElementTypeID;
}

function getElementIDFromKanbanTask($KanbanTaskID)
{
    global $conn;
    global $functions;
...
function updateKanbanTaskDeadline($Deadline, $ElementID, $ModuleID)
{
    ...
    $sql = "UPDATE taskslist SET Deadline = '$Deadline'
            WHERE RelatedElementID = '$ElementID' AND RelatedElementTypeID = $ModuleID";
...
function updateKanbanTaskResponsible($NewResponsible, $OldResponsible, $ElementID, $ModuleID)
{
    ...
    //If new responsible does not have task
    if (empty($ExistingTaskID)) {

        $sql = "UPDATE taskslist SET RelatedUserID = '$NewResponsible', Responsible = '$NewResponsible'
                WHERE RelatedElementID = '$ElementID' AND RelatedElementTypeID = '$ModuleID' AND RelatedUserID = '$OldResponsible'
                AND Status != '4'";

        mysqli_query($conn, $sql) or die(mysqli_error($conn));
    }

    $sql = "UPDATE taskslist SET Responsible = '$NewResponsible'
            WHERE RelatedElementID = '$ElementID' AND RelatedElementTypeID = '$ModuleID'
            AND Status != '4'";
...
function getDeadlineForChange($ElementID)
{
    global $conn;
    global $functions;
    $sql = "SELECT ChangeEnd
            FROM changes
            WHERE ID = $ElementID";
...
function closeKanBanTasks($ElementID, $ModuleID)
{
    ...
    $sql = "UPDATE taskslist
            SET Status = '3'
            WHERE RelatedElementID = $ElementID AND RelatedElementTypeID = $ModuleID AND taskslist.Status != 4";
...
function getScreenShots($ElementID, $ModuleID)
{
    global $conn;
    global $functions;

    $sql = "SELECT ID,PictureName,Location,Created,CreatedBy
            FROM comment_pictures
            WHERE ElementID = '$ElementID' AND ModuleID = '$ModuleID' AND Comment = '0'
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $Array[] = array('ID' => $row['ID'], 'PictureName' => $row['PictureName'], 'Location' => $row['Location'], 'Created' => $row['Created'], 'CreatedBy' => $row['CreatedBy']);
    }
...
function addUserToRole($Username, $RoleID)
{
    ...
    $sql = "INSERT INTO usersroles (UserID,RoleID) VALUES ($RetrievedUserID,$RoleID)";
...
function removeUsersFromRole($RoleID)
{
    ...
    $sql = "DELETE FROM usersroles WHERE RoleID = '$RoleID' AND UserID != '1'";
...
function createTeam($TeamName)
{
    ...
    $sql = "INSERT INTO teams (Teamname,Colour) VALUES ('$TeamName','1')";
...
function deleteTeamRelations($TeamID)
{
    ...
    $sql = "DELETE FROM usersteams WHERE TeamID = '$TeamID'";
...
function insertUserOnTeam($UserID, $TeamID)
{
    ...
    $sql = "INSERT INTO usersteams (UserID,TeamID) VALUES ('$UserID','$TeamID')";
...
function updateITSMFieldValue($FieldID, $FieldValue, $FieldName, $ITSMTypeID)
{
    ...
    $sql = "UPDATE $ITSMTableName SET $FieldName = '$FieldValue'
            WHERE ID = $FieldID";
...
function getNextFreeIDFromTable($Table)
{
    global $conn;
    global $functions;
...
function duplicateFormEntry($ITSMID, $NewITMSID, $FormTableName){
    ...
    $sql = "INSERT INTO $FormTableName($Temp)
            SELECT $Temp
            FROM $FormTableName
            WHERE RelatedRequestID = $ITSMID;";
    ...
    $NewFormNetryID = mysqli_insert_id($conn);

    $sql = "UPDATE $FormTableName
            SET RelatedRequestID = $NewITMSID
            WHERE ID = $NewFormNetryID;";
...
function updateRelatedRequestOnFormFieldRecord($TableName, $RecordID, $RequestID)
{
    ...
    $sql = "UPDATE $TableName SET RelatedRequestID = '$RequestID' WHERE ID = '$RecordID';";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function createNewRequestFromFormView($RequestID, $FormID, $RecordID, $UserSessionID, $RequestCustomer, $BusinessService, $RequestSubject, $RequestProblemText, $Priority, $RequestRespTeam, $RequestRespSup, $Base64Paste)
{
    ...
    $CustomerName = $functions->getUserFullName($RequestCustomer);
    $SlackStatus = $functions->getSettingValue(34);
    if ($SlackStatus == 1) {
        $Webhook = getSlackWebhookNewProblem();
        $functions->sendMessageToSlack("Ticket recieved on practicle from $CustomerName: <https://practicle.practicle.dk/incidents_view.php?elementid=$NewRequestID|$RequestSubject>", $Webhook);
    }
    return $RequestID;
}

function setServerInactive($RelatedCI)
{
    global $conn;
    global $functions;

    $sql = "UPDATE cis SET Active = '0' WHERE ID = '$RelatedCI';";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function updateCIRelationLookup($FieldID, $NewStringValue)
{
    ...
    $sql = "UPDATE cmdb_ci_fieldslist SET RelationsLookup = '$NewStringValue'
            WHERE ID = $FieldID";
    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
}
...
function updateSelectOptions($FieldID, $NewStringValue, $TableName)
{
    ...
    $sql = "UPDATE $TableName SET SelectFieldOptions = '$NewStringValue'
            WHERE ID = $FieldID";
...
function updateGroupFilter($FieldID, $NewStringValue, $TableName)
{
    ...
    $sql = "UPDATE $TableName SET GroupFilterOptions = '$NewStringValue'
            WHERE ID = $FieldID";
...
function insertITSMFieldToFieldListTable(
    ...
    try {
        ...
        $functions->errorlog($e->getMessage(),"insertITSMFieldToFieldListTable");
    }
}
...
function deleteCIFromCMDBCITable($CIID)
{
    ...
    $sql = "DELETE FROM cmdb_cis WHERE ID = $CIID;";
...
function deleteITSMFromITSMTable($ITSMID)
{
    ...
    $sql = "DELETE FROM itsm_modules WHERE ID = $ITSMID;";
...
function deleteCIFields($CIID)
{
    ...
    $sql = "DELETE FROM cmdb_ci_fieldslist WHERE RelatedCITypeID = $CIID;";
...
function deleteITSMFields($ITSMID)
{
    ...
    $sql = "DELETE FROM itsm_fieldslist WHERE RelatedTypeID = $ITSMID;";
...
function deleteCILogs($CITypeID)
{
    ...
    $sql = "DELETE FROM log_cis WHERE RelatedType = $CITypeID;";
...
function deleteITSMLogs($ITSMTypeID)
{
    ...
    $sql = "DELETE FROM itsm_log WHERE RelatedType = $ITSMTypeID;";
...
function deleteITSMStatusCodes($ITSMTypeID)
{
    ...
    $sql = "DELETE FROM itsm_statuscodes WHERE ModuleID = $ITSMTypeID;";
...
function deleteITSMSLAMatrix($ITSMTypeID)
{
    ...
    $sql = "DELETE FROM itsm_sla_matrix WHERE RelatedModuleID = $ITSMTypeID;";
...
function deleteITSMSLATimelines($ITSMTypeID)
{
    ...
    $sql = "DELETE FROM itsm_slatimelines WHERE RelatedElementTypeID = $ITSMTypeID;";
...
function deleteITSMParticipants($ITSMTypeID)
{
    ...
    $sql = "DELETE FROM itsm_participants WHERE ModuleID = $ITSMTypeID;";
...
function deleteITSMTasks($ITSMTypeID)
{
    ...
    $sql = "DELETE FROM taskslist WHERE RelatedElementTypeID = $ITSMTypeID;";
...
function deleteITSMWorkFlows($ITSMTypeID)
{
    ...
    // First, delete associated workflow steps
    $sqlDeleteSteps = "DELETE FROM workflowsteps 
                       WHERE RelatedWorkFlowID IN (SELECT ID FROM workflows WHERE RelatedElementTypeID = $ITSMTypeID)";

    $resultDeleteSteps = mysqli_query($conn, $sqlDeleteSteps) or die('Error deleting workflow steps: ' . mysqli_error($conn));

    // Then, delete workflows
    $sqlDeleteWorkflows = "DELETE FROM workflows WHERE RelatedElementTypeID = $ITSMTypeID";

    $resultDeleteWorkflows = mysqli_query($conn, $sqlDeleteWorkflows) or die('Error deleting workflows: ' . mysqli_error($conn));
}
...
function deleteCIRelations($CITableName)
{
    ...
    $sql = "DELETE FROM cmdb_ci_relations WHERE CITable1 = '$CITableName' OR CITable2 = '$CITableName';";
...
function deleteITSMRelations($ITSMTableName)
{
    ...
    $sql = "DELETE FROM itsm_relations WHERE Table1 = '$ITSMTableName' OR Table2 = '$ITSMTableName';";
...
function deleteCIFiles($CITypeID)
{
    ...
    $sql = "SELECT FileName
            FROM files_cis
            WHERE RelatedType = $CITypeID";

    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

    while ($row = mysqli_fetch_array($result)) {
        $FileName = $row['FileName'];
        $file_pointer = "../uploads/files_cis/$FileName";
    ...
    $sql = "DELETE FROM files_cis WHERE RelatedType = $CITypeID;";
...
function deleteITSMFiles($ITSMTypeID)
{
    ...
    $sql = "SELECT FileName
            FROM files_itsm
            WHERE RelatedType = $ITSMTypeID";

    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

    while ($row = mysqli_fetch_array($result)) {
        $FileName = $row['FileName'];
        $file_pointer = "../uploads/files_itsm/$FileName";
    ...
    $sql = "DELETE FROM files_itsm WHERE RelatedType = $ITSMTypeID;";
...
function removeAllCIRelations($TableName)
{
    ...
    try {
        $sql = "DELETE FROM cmdb_ci_relations
                WHERE CITable1 = ? OR CITable2 = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ss", $TableName, $TableName);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    } catch (Exception $e) {
...
function doesCIExist($TableName, $CIIDFieldName, $IDColumn, $CITypeID)
{
    ...
    // 1. Get all relevant fields from cmdb_ci_fieldslist table
    $fieldQuery = "SELECT FieldName FROM cmdb_ci_fieldslist 
                   WHERE RelatedCITypeID = '$CITypeID' 
                   AND DefaultField = 0;";

    $fieldResult = mysqli_query($conn, $fieldQuery) or die('Field query failed: ' . mysqli_error($conn));
    ...
    // 2. Construct the dynamic SQL query to select the record from the target table
    $sql = "SELECT $CIIDFieldName, " . implode(", ", $fields) . " 
            FROM $TableName 
            WHERE $CIIDFieldName = '$IDColumn';";
    ...
    if (mysqli_num_rows($result) == 0) {
        // No record found, so return an empty string to indicate creation
        return "";
    }

    $row = mysqli_fetch_assoc($result);

    // 3. Compare the existing data with the new data
    $differences = 0;
    foreach ($fields as $field) {
        if (isset($_POST[$field])) {  // Assuming the new data comes from POST request
    ...
    // 4. If 3 or more fields are different, treat it as a new record
    if ($differences >= 3) {
...
function setEndDateOnCI($CITableName, $CITypeID, $CIID)
{
    ...
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    $LogActionText = "CI EndDate set to $Date by sync job";
    createCILogEntry($CIID, $CITypeID, "1", $LogActionText);
}
...
function setStartDateOnCI($CITableName, $CITypeID, $CIID)
{
    ...
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    $LogActionText = "CI StartDate set to $Date by sync job";
    createCILogEntry($CIID, $CITypeID, "1", $LogActionText);
}
...
function getITSMLookupField($ITSMTypeID)
{
    ...
    $sql = "SELECT FieldLabel
            FROM itsm_fieldslist
            WHERE RelatedTypeID = '$ITSMTypeID' AND RelationShowField = '1';";
...
function getCIFields($CIID)
{
    ...
    $sql = "SELECT FieldName, FieldLabel
            FROM cmdb_ci_fieldslist
            WHERE RelatedCITypeID = $CIID AND DefaultField = 0
            ORDER BY FieldOrder ASC";

    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));

    while ($row = mysqli_fetch_array($result)) {
        $Array[] = array('FieldName' => $row['FieldName'], 'Label' => $row['FieldLabel']);
    }
    return $Array;
}
...
function getAllActiveCIs($CITypeID){
    ...
    while ($row = mysqli_fetch_array($result)) {
        $CIID = $row["ID"];
        $FieldName = $row["$CIIDFieldName"];
        $Array[] = array("ID" => $CIID, "Value" => $FieldName);
    }
...
/*
function getITSMRelationShowField($ITSMTypeID)
{
    global $conn;
    global $functions;
...
function checkifCIRelationExists($CITable1, $CITable2, $CI1ID, $CI2ID)
{
    ...
    $sql = "SELECT ID
            ...
            WHERE (CITable1 = '$CITable1' AND CITable2 = '$CITable2' AND CI1ID = '$CI1ID' AND CI2ID = '$CI2ID')
            OR (CITable1 = '$CITable2' AND CITable2 = '$CITable1' AND CI1ID = '$CI2ID' AND CI2ID = '$CI1ID');";
...
function createOrUpdateCIRelation($CITable1, $CITable2, $CI1ID, $CI2ID, $CIName1, $CIName2, $CIValue1, $CIValue2, $CITypeID1, $CITypeID2)
{
    ...
    try {
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssii", $CITable1, $CITable2, $CI1ID, $CI2ID);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
...
function updateCISyncDateTime($ID)
{
    ...
    $sql = "UPDATE cmdb_cis SET LastSyncronized = Now() WHERE ID = '$ID';";
...
function createStatusSLAEntries($ModuleID, $StatusID)
{
    ...
    $sql1 = "INSERT INTO `itsm_sla_matrix`(`RelatedModuleID`, `Status`, `SLA`, `P1`, `P2`, `P3`, `P4`) 
            VALUES ('$ModuleID','$StatusID','1','0','0','0','0');";
    
    mysqli_query($conn, $sql1);

    $sql2 = "INSERT INTO `itsm_sla_matrix`(`RelatedModuleID`, `Status`, `SLA`, `P1`, `P2`, `P3`, `P4`) 
            VALUES ('$ModuleID','$StatusID','2','0','0','0','0');";

    mysqli_query($conn, $sql2);

    $sql3 = "INSERT INTO `itsm_sla_matrix`(`RelatedModuleID`, `Status`, `SLA`, `P1`, `P2`, `P3`, `P4`) 
            VALUES ('$ModuleID','$StatusID','3','0','0','0','0');";
...
function deleteStatusSLAEntries($ModuleID, $StatusID)
{
    ...
    $sql = "DELETE FROM itsm_sla_matrix WHERE RelatedModuleID = '$ModuleID' AND Status = '$StatusID';";
...
function getSLAStatusCores($ModuleID, $SLA, $Priority)
{
    ...
    $sql = "SELECT Status, P$Priority
            ...
            WHERE RelatedModuleID = $ModuleID AND SLA = $SLA;";
    ...
    while ($row = mysqli_fetch_array($result)) {
        $Status = $row[0];
        $Minutes = $row[1];
        $array[] = array("Status" => $Status, "Minutes" => $Minutes);
    }
    mysqli_free_result($result);
    return $array;
}
...
function createITSMFilesFromTemp($ITSMID, $ITSMTypeID, $UserID){
    ...
    $sql1 = "SELECT FileName, FileNameOriginal, TempPath
            FROM files_temp
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sql2 = "INSERT INTO files_itsm (FileName,FileNameOriginal,RelatedElementID,RelatedUserID,RelatedType)
                VALUES ('$FileName','$FileNameOriginal','$ITSMID','$UserID','$ITSMTypeID');";

        mysqli_query($conn, $sql2);

        $sql3 = "DELETE FROM files_temp WHERE RelatedUserID = $UserID;";
...
function removeClosedStatus($ITSMTypeID, $StatusID){
    ...
    $sql1 = "UPDATE itsm_statuscodes SET ClosedStatus = 0
            WHERE ID != $StatusID AND ModuleID = $ITSMTypeID";
...
function getITSMStatusCodesBelowFinishedStatus($ITSMTypeID, $FinishedStatus)
{
    ...
    $sql = "SELECT StatusCode
            FROM itsm_statuscodes
            WHERE itsm_statuscodes.ModuleID = $ITSMTypeID AND SLA = 1 AND StatusCode < $FinishedStatus
            ORDER BY StatusCode ASC;";
...
function getITSMStatusCodesAllreadyUpdated($ITSMTypeID, $ITSMID)
{
    ...
    $sql = "SELECT RelatedStatusCodeID
            FROM itsm_slatimelines
            WHERE itsm_slatimelines.TimelineUpdatedDate IS NOT NULL AND itsm_slatimelines.RelatedElementID = $ITSMID AND itsm_slatimelines.RelatedElementTypeID = $ITSMTypeID
            ORDER BY RelatedStatusCodeID ASC;";
    ...
    while ($row = mysqli_fetch_array($result)) {
        $ValueArray[] = ($row["RelatedStatusCodeID"]);
    }
...
function getCertificateExpireDate($URL)
{
    ...
    $valid_to = strtotime(date(DATE_RFC2822, $certinfo['validTo_time_t']));
    $timestamp = date('Y-m-d H:m:s', $valid_to);
    $UpdateUrl = "<a href=\"javascript:updateCertificateEndDate('$timestamp');\">Click to update</a>";
    return $timestamp." - $UpdateUrl";   
}
...
function getCertificateExpireDateAuto($URL)
{
    ...
    if ($read === false) {
        // Could not establish connection within the timeout period
        return false;
    }
    ...
    $certinfo = openssl_x509_parse($cert['options']['ssl']['peer_certificate']);
    $valid_to = strtotime(date(DATE_RFC2822, $certinfo['validTo_time_t']));
    $timestamp = date('Y-m-d H:i:s', $valid_to); // Corrected the date format (H:i:s)
...
function updateCertificateEndDate($ciTypeId, $ciId, $endDate){
    ...
    $sql = "UPDATE $CITableName SET EndDate = '$endDate'
            WHERE ID = '$ciId';";
...
function getModuleTemplateSQL($TemplateID, $Type)
{
    ...
    if ($result = mysqli_query($conn, $sql)) {
        ...
        $functions->errorlog("Query fail: " . mysqli_error($conn), "getModuleTemplateSQL");
    }
...
function closeAllTasksAssociatedWithITSM($ITSMTypeID, $ITSMID)
{
    ...
    $checkSql = "SELECT COUNT(*) AS count FROM taskslist
                 WHERE RelatedElementTypeID = '$ITSMTypeID' AND RelatedElementID = '$ITSMID' AND Status = '4';";
    $checkResult = mysqli_query($conn, $checkSql) or die('Query fail: ' . mysqli_error($conn));
    ...
    $updateSql = "UPDATE taskslist SET Status = '3'
                  WHERE RelatedElementTypeID = '$ITSMTypeID' AND RelatedElementID = '$ITSMID';";
    mysqli_query($conn, $updateSql) or die('Query fail: ' . mysqli_error($conn));
}
...
function getFormFieldIDFromFieldOrder($TableName, $FormID, $OrderID)
{
    ...
    $sql = "SELECT ID
            FROM $TableName
            WHERE RelatedFormID = $FormID AND FieldOrder = $OrderID;";
...
function getFieldIDFromFieldOrder($TableName, $ITSMTypeID, $OrderID)
{
    ...
    $sql = "SELECT ID
            FROM $TableName
            WHERE RelatedTypeID = $ITSMTypeID AND FieldOrder = $OrderID;";
...
function updateParentTaskIDOnTask($TaskID, $ParentTaskID)
{
    ...
    $sql = "UPDATE project_tasks SET ParentTask = $ParentTaskID
            WHERE ID = $TaskID;";
...
function getCMDBTypes()
{
    ...
    $sql = "SELECT cmdb_cis.ID, cmdb_cis.Name, cmdb_cis.TableName, cmdb_ci_fieldslist.FieldName
            FROM cmdb_cis
            LEFT JOIN cmdb_ci_fieldslist ON cmdb_ci_fieldslist.RelatedCITypeID = cmdb_cis.ID AND cmdb_ci_fieldslist.RelationShowField = 1
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $TableName = $row["TableName"];
        $FieldName = $row["FieldName"];
        $resultarray[] = array("CMDBTypeID" => $CMDBTypeID, "Name" => $Name, "TableName" => $TableName, "FieldName" => $FieldName);
    }
...
function getExpiredForCMDBType($CMDBTypeID, $Name, $TableName, $FieldName, $UserSessionID)
{
    ...
    if (
        !columnExists($TableName, 'RelatedUserID') ||
        !columnExists($TableName, 'Active') ||
        !columnExists($TableName, 'EndDate')
    ) {
        return array(); // Return an empty array if any column is missing
    }

    $sql = "SELECT ID, $FieldName, EndDate
            FROM $TableName
            WHERE $TableName.RelatedUserID = '$UserSessionID' AND $TableName.Active = 1 AND EndDate <= DATE_ADD(NOW(), INTERVAL 10 DAY);";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $Link = "javascript:runModalViewCI('$ID','$CMDBTypeID','1');";
        $resultarray[] = array("TypeName" => $Name, "CIName" => $CIName, "Link" => $Link, "EndDate" => $EndDate);
    }
...
function incrementExistingFieldOrder($ITSMTypeID, $FieldOrder)
{
    ...
    $sql = "UPDATE itsm_fieldslist
            SET FieldOrder = FieldOrder + 1
            WHERE RelatedTypeID = '$ITSMTypeID' AND FieldOrder >= $FieldOrder;";
...
function incrementExistingCIFieldOrder($CITypeID, $FieldOrder)
{
    ...
    $sql = "UPDATE cmdb_ci_fieldslist
            SET FieldOrder = FieldOrder + 1
            WHERE RelatedCITypeID = '$CITypeID' AND FieldOrder >= $FieldOrder;";
...
function logActivity($ElementID, $ModuleID, $Headline, $Text, $Url)
{
    ...
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssisss", $ElementID, $ModuleID, $UserID, $Headline, $Text, $Url);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
}
...
function getAllTimeRegistrations($week)
{
    ...
    $LeaderOf = implode(",", $LeaderOfArray);

    $sql = "SELECT
        users.ID AS UserID,
        CONCAT(users.Firstname,' ',users.Lastname, ' (',users.UserName,')') AS FullName, users.UserName";

    $daysOfWeek = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
    foreach ($daysOfWeek as $day) {
        $sql .= ", FORMAT(SUM(CASE WHEN DAYNAME(time_registrations.DateWorked) = '$day' THEN time_registrations.TimeRegistered / 60 ELSE 0 END), 2) AS $day";
    }

    $sql .= " FROM time_registrations
    ...
    if ($result = mysqli_query($conn, $sql)) {
        while ($row = mysqli_fetch_array($result)) {
            ...
            $resultarray[] = array("UserID" => $UserID, "UserName" => $UserName, "ProfilePopOver" => $ProfilePopOver) + array_slice($row, 2);
        }
        mysqli_free_result($result);
    } else {
        $functions->errorlog("Query fail: " . mysqli_error($conn), "getAllTimeRegistrations");
    }
    return $resultarray;
}
...
function getITSMAsMailWithPDFFields($ITSMTypeID, $ITSMID, $UserID, $ModuleType, $UserLanguageCode, $Type, $group_array)
{
    global $conn;
    global $functions;
    ...
    // Add a line break marker to $FieldsArray1 only if it's not empty and $FieldsArray2 is also being populated
    if (!empty($FieldsArray1)) {
        $FieldsArray1[] = array("FieldLabel" => "/n", "FieldValue" => "/n");
        // Loop through the first array and extract the desired fields
        foreach ($FieldsArray1 as $item) {
            $mergedArray[] = extractITSMFormFieldsArray($item);
        }
    }

    // Loop through the second array and extract the desired fields
...
function getITSMRelatedFormID($ITSMTypeID, $ITSMID)
{
    ...
    // Attempt to fetch a row to check if the table and column exist
    $testSql = "SHOW COLUMNS FROM `$ITSMTableName` LIKE 'RelatedFormID';";
    $testResult = mysqli_query($conn, $testSql);

    // Check if the column exists by checking the number of rows returned
    if (mysqli_num_rows($testResult) == 0) {
        // Column does not exist, return a default value
        return "";
    }

    // Proceed with the original query if the column exists
    $sql = "SELECT RelatedFormID FROM `$ITSMTableName` WHERE ID = $ITSMID;";
    $result = mysqli_query($conn, $sql);
    ...
    if ($result) {
        ...
        if (isset($RelatedFormID) && !empty($RelatedFormID)) {
            return $RelatedFormID;
        }
    }
...
function getITSMFieldsValues($ITSMTypeID, $ITSMID, $ModuleType, $UserLanguageCode, $Type, $group_array, $getPDF)
{
    ...
    $sql = "SHOW COLUMNS FROM $ITSMTableName";
    $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        // This is a PDF generator job - lets get real values for select options
        if($FieldType == "4" && $FieldValue !== "" && $getPDF == "1"){
            // Replace bold tags
            $FieldValue = str_replace(array('<b>', '</b>'), array('**', '**'), $FieldValue);
            // Replace italic tags
            $FieldValue = str_replace(array('<i>', '</i>'), array('*', '*'), $FieldValue);
            // Replace break tags with newline
            $FieldValue = str_replace(array('<br>', '<br/>', '<br />'), "\n", $FieldValue);
            // Replace paragraph tags with double newline
            $FieldValue = str_replace(array('<p>', '</p>'), array('', "\n\n"), $FieldValue);
            ...
            if(!empty($FieldSelectOptions)){
                ...
                $sql = "SELECT $FieldResultView
                        ...
                        WHERE $FieldLookupField = '$FieldValue'";                

                $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
                while ($row = mysqli_fetch_array($result)) {
                    $FieldValue = $row["$FieldResultView"];
                }
            }
        }
        
        $HideField = $functions->getITSMFieldHideStatus($FieldName, $ITSMTypeID);
        $FieldType = $functions->getITSMFieldType($ITSMTypeID, $FieldName);
        ...
        if ("$FieldName" == "$RelationFieldName") {
            ...
            if ($Type == "quickview") {
                $RelationFIeld = "<a href='javascript:location.reload(true);'>$RelationFIeld</a>";
                $group_array = $_SESSION['memberofgroups'];
                if (in_array("100001", $group_array) || in_array("100015", $group_array)) {
                    $RelationFIeld = $RelationFIeld . navMenu01($ITSMTypeID);
                }
            }
        } else {
            $RelationFIeld = "0";
        ...
        $FieldsArray[] = array("ModuleType" => $ModuleType, "LanguageCode" => $UserLanguageCode, "FieldType" => $FieldType, "FieldName" => $FieldName, "FieldValue" => $FieldValue, "RelationFIeld" => $RelationFIeld, "ITSMTypeName" => $ITSMTypeName, "FieldLabel" => $FieldLabel);
    }
...
function getRequestFormFieldValues($ITSMTypeID, $ITSMID, $getPDF)
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (!in_array($FieldName, $Excludes)) {
            ...
            $FieldType = $functions->getFormsFieldTypeID($ITSMFormID, $FieldName);
            ...
            $FieldsArray[] = array("FieldType" => $FieldType, "FieldName" => $FieldName, "FieldLabel" => $FieldLabel, "FieldValue" => $FieldValue);
        }
    }
    return $FieldsArray;
}
