...
if (isset($_GET['getMessageInformations'])) {
  ...
  while ($row = mysqli_fetch_array($result)) {
    $FromUser = getFullNameFromID($row['FromUserID']);
...
if (isset($_GET['addWorkFlowStep'])) {
  ...
  $CustomerName = getFullNameFromID(getElementCustomerID($ElementID, $ElementType));
...
if (isset($_GET['createticketfromimap'])) {
  getMailsAndCreateTickets();
}

if (isset($_GET['createticketfromimap_old'])) {
  $UID = $_GET["UID"];
  include_once('./vendor/autoload.php');
  include_once('./classes/imap/class.imap.php');
  $EmailExists = "";
  $IMAPServer = "";
  $IMAPMailAddress = "";
  $IMAPPassword = "";
...
if (isset($_GET['getMailsAndCreateTickets'])) {
  include_once('./vendor/autoload.php');
  include_once('./classes/imap/class.imap.php');
  $EmailExists = "";
  $IMAPServer = "";
...
if (isset($_GET['orderNewSiteInstall'])) {
  ...
  $ExistingSiteName = doesORSiteExist($SiteName);
...
if (isset($_GET['getCIDatarows'])) {
  ...
  $tmp = "Actions";
  $SelectHeader = "SELECT $CITableName.ID,'' AS $tmp,";
  $SelectButtom = "";
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    $CIID = $row['ID'];
    $NewValue = "<a href=\"javascript:runModalViewCI('$CIID','$CITypeID','$AllowDelete');\"><span class=\"badge bg-gradient-success\"><i class=\"fa fa-pen-to-square\"></i></span></a>";
...
if (isset($_GET['getITSMDatarowsMy'])) {
  ...
  try {
    ...
    // Bind parameters
    if (!$stmt->bind_param("s", $ITSMTypeID)) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    if($numberOfUnreadComments > 0){
      //$unreadCommentsElement = "<span class=\"badge badge-pill bg-gradient-warning\" title=\"". $functions->translate("Unread comments")."\"><i class=\"fa-regular fa-envelope\"></i></span>";
    ...
    if ($screensize == "sm") {
      $Temp = $row["$FieldPrimary"];
      if (strlen($Temp) > 22) {
        $Temp = substr($Temp, 0, 22) . '...';
      }
      $NewValue = "<a href=\"javascript:viewITSM('$ITSMID','$ITSMTypeID','$AllowDelete','modal');\">$Temp</a>"." ".$unreadCommentsElement;
      $row["$FieldPrimary"] = $NewValue;
    } else {
      $Temp = $row["$FieldPrimary"];
      $NewValue = "<a href=\"javascript:viewITSM('$ITSMID','$ITSMTypeID','$AllowDelete','modal');\">$Temp</a>"." ".$unreadCommentsElement;
...
if (isset($_GET['sendITSMAsMailWithPDF'])) {
  ...
  $Result = getITSMAsMailWithPDFFields($ITSMTypeID, $ITSMID, $UserID, $ModuleType, $UserLanguageCode, $Type, $group_array);
  
  $mpdf = new \Mpdf\Mpdf();

   // Set document information
  $mpdf->SetTitle("Form $ITSMTypeName: $ITSMID");
  // Initialize HTML content with a title
  $htmlContent = "<br><br><br><br><h1>$ITSMTypeName: $ITSMID</h1>";

  // Iterate over the $Result array to add each entry to the PDF
  foreach ($Result as $field) {
    // Check if FieldLabel is set and not empty
    if (!empty($field["FieldLabel"])) {
      // Append the field information to the HTML content
  ...
  // Specify the directory where the PDF will be saved
  $pdfDir = __DIR__ . '/uploads/files_itsm'; // Using __DIR__ for an absolute path
  ...
  try {
    ...
    $functions->errorlog($error, "sendITSMAsMailWithPDF: sendMailToSinglePerson dailed to create PDF");
    $response[] = array("Result" => "error", "Message" => "Failed to create PDF: $error");
  ...
  if ($selectedEmail == "") {
    ...
    $sql = "INSERT INTO files_itsm (FileName, FileNameOriginal, RelatedElementID, Date, RelatedUserID, RelatedType, FileContent)
            VALUES ('$pdfName','$pdfName','$ITSMID',NOW(),'$SessionUserID','$ITSMTypeID',NULL);";

    mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
  } else {
    $EmailSubject = "$ITSMTypeName: $ITSMID";
    if ($PDFChoice == "1") {
      // Save the PDF to a file
      try {
        $emailResult = sendMailToSinglePerson($selectedEmail, $selectedEmail, $EmailSubject, $htmlContent, $pdfPath, $pdfName);
      } catch (Exception $e) {
        $error = $e->getMessage();
        // Handle the exception
        $functions->errorlog($error, "sendITSMAsMailWithPDF: Failed to email PDF file");
        $response[] = array("Result" => "error", "Message" => "Failed to email PDF file: $error");
        echo json_encode($response);
        exit;
      }
    } else {
      try {
        $emailResult = sendMailToSinglePerson($selectedEmail, $selectedEmail, $EmailSubject, $htmlContent);
      } catch (Exception $e) {
        $error = $e->getMessage();
        // Handle the exception
        $functions->errorlog($error, "sendITSMAsMailWithPDF: sendMailToSinglePerson failed to send email");
        $response[] = array("Result" => "error", "Message" => "Failed to send email: $error");
        echo json_encode($response);
        exit;
      }
    }
  ...
  $response[] = array("Result" => "success", "ITSMID" => $ITSMID, "ITSMTypeID" => $ITSMTypeID);
  echo json_encode($response);
}
...
if (isset($_GET['getResponsiblesOpenITSM'])) {
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    $ResponsibleName = getFullNameFromID($Responsible);
