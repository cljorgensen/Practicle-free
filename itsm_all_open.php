...
<script>
	$(document).ready(function() {
		...
		$.ajax({
			type: "POST",
			url: "./getdata.php?getITSMDatarowsAllOpen",
			data: vData,
			dataType: 'JSON',
			success: function(data) {
				...
				if (rowData) {
					...
					var table = $('#itsmtableallopen').DataTable({
						"dom": 'Bfrtip',
						"bFilter": true,
						"paging": true,
						"info": false,
						...
						initComplete: function() {
							$('#itsmtableallopen thead th').each(function() {
								var title = $(this).text();
							...
							<li class="nav-item dropdown pe-2">
								...
								<ul class="dropdown-menu dropdown-menu-end p-2 me-sm-n4" aria-labelledby="dropdownMenuButton">
									...
									foreach ($BSArray as $key => $value) {
										$Name = $value["Name"];
										echo "<li class=\"mb-2\">
												<a class=\"dropdown-item border-radius-md\" onclick=\"window.location.href='itsm_quickview.php?id=" . $value['ID'] . "'\">
				...
				<div class="card-body">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="tab-pane fade show active" id="My" role="tabpanel" aria-labelledby="My">
							<table id="itsmtableallopen" class="table table-borderless table-responsive table-hover" cellspacing="0">
							</table>
						</div>
					</div>
				</div>
