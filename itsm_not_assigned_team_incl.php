...
<script>
	$(document).ready(function() {
		...
		<?php initiateAdancedViewTable("dispatchqueticketsnoteam"); ?>

	});
</script>
<!-- the table view -->
<table id="dispatchqueticketsnoteam" class="table table-responsive table-borderless table-hover" cellspacing="0">
	<thead>
		<tr>
			<th><?php echo _('ID'); ?></th>
			<th></th>
	...
	$sql = "SELECT $ITSMTableName.ID AS ITSMID, $ITSMTableName.Subject, $ITSMTableName.Description, $ITSMTableName.Status, $ITSMTableName.Priority, $ITSMTableName.Created, $ITSMTableName.Customer, $ITSMTableName.Responsible, $ITSMTableName.Team, companies.Companyname, teams.Teamname
			...
			WHERE Team IS NULL AND Status NOT IN (1,6,7,8)
	...
	<tbody>
		<?php while ($row = mysqli_fetch_array($result)) { ?>
			...
			$CustomerName = getFullNameFromID($CustomerID);
			$ResponsibleName = getFullNameFromID($ResponsibleID);
			$AllowDelete = "0";
