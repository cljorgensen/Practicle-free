    ...
    var table = $('#TableProjectTasks').DataTable({
      ...
      "columnDefs": [{
          ...
          "render": function(data, type, row, meta) {
            ...
            let cleanText1 = '';
            if (RelatedTaskCategoryName) {
              cleanText1 = TaskCategoryDescription.replace(/<\/?[^>]+(>|$)/g, "");
            } else {
              cleanText1 = 'none';
              RelatedTaskCategoryName = '';
            }

            let fieldtext1 = '<span data-bs-toggle="tooltip" data-bs-title="' + cleanText1 + '">' + RelatedTaskCategoryName + '</span>';
            return fieldtext1;
          }
        }, {
          "targets": 2,
          "data": null,
          "render": function(data, type, row, meta) {
            let ClosedStatus = row['ClosedStatus'];
            if (ClosedStatus == '1') {
              return '<i class="fa-solid fa-lock" data-bs-toggle="tooltip" data-bs-title="<?php echo _("This task is private and you are not added as project task participant") ?>"></i>';
            } else {
              let parentTaskButton = '';
              if (row['ParentTask'] && row['ParentTask'] !== -1) {
                let parentTaskName = row['ParentTaskName'];
                parentTaskButton = '<a href="javascript:runModalEditProjectTask(' + row['ParentTask'] + ',' + row['ProjectID'] +
                  ');"><span class="badge badge-pill bg-gradient-secondary" data-bs-toggle="tooltip" data-bs-title="Parent Task is: ' + parentTaskName + '"><i class="fa-solid fa-diagram-predecessor"></i></span></a> ';
              }
              return '<a href="projects_tasks_view.php?projecttaskid=' + row['TaskID'] + '"><span class="badge badge-pill bg-gradient-info" data-bs-toggle="tooltip" data-bs-title="Open"><i class="fas fa-folder-open"></i></span></a> ' +
                '<a href="javascript:runModalEditProjectTask(' + row['TaskID'] + ',' + row['ProjectID'] +
                ');"><span class="badge badge-pill bg-gradient-success" data-bs-toggle="tooltip" data-bs-title="View"><i class="fa-solid fa-pen-to-square"></i></span></a>' + parentTaskButton;
            }
          }
        },
        {
          ...
          "render": function(data, type, row, meta) {
            var indent = '';
            var depth = row['TaskDepth'];
            for (var i = 0; i < depth; i++) {
              indent += '&nbsp;&nbsp;&nbsp;&nbsp;';
            }

            var descriptiontext = row['Description'];
            if (descriptiontext) {
              var cleanText2 = descriptiontext.replace(/<\/?[^>]+(>|$)/g, "");
              var TaskName = row['TaskName'];
              var fieldtext2 = '<span data-bs-toggle="tooltip" data-bs-title="' + cleanText2 + '">' + indent + TaskName + '</span>';
              return '<a href="projects_tasks_view.php?projecttaskid=' + row['TaskID'] + '">' + fieldtext2 + '</a>';
            } else {
              var TaskName = row['TaskName'];
              var fieldtext2 = '<span data-bs-toggle="tooltip" data-bs-title="">' + indent + TaskName + '</span>';
              return '<a href="projects_tasks_view.php?projecttaskid=' + row['TaskID'] + '">' + fieldtext2 + '</a>';
            }
          }
        },
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        for (var i = 0; i < obj.length; i++) {
          ...
          if (ParentTask === "-1" || ParentTask === null) {
            ParentTask = ""; // Or any other action you'd like to take
          } else {
...
<script>
  $(document).ready(function() {
    ...
    <?php initiateAdancedViewTable("TableProjectLog"); ?>
  });
</script>
<!-- Kanban scripts -->
